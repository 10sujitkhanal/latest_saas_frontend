...
export default function AgentTrainer({ workspaceId }: { workspaceId: string | number }) {
  ...
  const createNew = async () => {
    ...
    setBusy(true);
    try {
      const r = await AgentsService.createProfile(workspaceId, { agent_type: 'crm', name: 'New CRM agent', instructions: '' });
      if (r.success && r.data) { toast.success('Agent created.'); await load(r.data.id); }
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <button type="button" onClick={createNew} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 px-2.5 py-1.5 text-xs font-semibold text-slate-200 disabled:opacity-50 shrink-0">
