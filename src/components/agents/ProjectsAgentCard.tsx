  ...
  return (
    <div className={embed ? '' : 'mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm'}>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <Link href={`/w/${workspaceId}/tasks?scope=all`} className="text-sm font-semibold text-slate-500 hover:text-slate-800">Open Tasks</Link>
      </div>
        ...
        <div className="mt-4 space-y-3">
          ...
          {data.overdue.length > 0 && (
            <div className="rounded-xl border border-rose-100 bg-rose-50/50 p-3">
              <p className="flex items-center gap-1.5 text-xs font-semibold text-rose-700"><AlertTriangle className="h-3.5 w-3.5" /> Overdue</p>
          ...
          {bullets.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
              <p className="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><Lightbulb className="h-3.5 w-3.5" /> What I&apos;d focus on</p>
              <ul className="mt-1.5 space-y-1">
                {bullets.map((b, i) => (
                  <li key={i} className="flex items-start gap-1.5 text-[13px] text-slate-700"><TrendingUp className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-500" /> {b}</li>
                ))}
              </ul>
            </div>
          )}
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3">
        <p className="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><ListChecks className="h-3.5 w-3.5" /> Break a goal into tasks</p>
        <div className="mt-2 flex items-center gap-2">
          <input value={goal} onChange={(e) => setGoal(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') breakdown(); }}
            ...
            className="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-900 outline-none focus:border-slate-400" />
          ...
          titles.length === 0 ? (
            ...
            <div className="mt-2">
              <ul className="space-y-1">
                {titles.map((t, i) => (
                  <li key={i} className="flex items-start gap-1.5 text-[13px] text-slate-700"><Plus className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-400" /> {t}</li>
                ))}
              </ul>
