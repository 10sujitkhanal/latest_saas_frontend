  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? <PageSkeleton kind="list" /> : error ? <ErrorBox message={error} onRetry={reload} /> : (
        <Card>
          <TableShell head={<tr><th className="px-3 py-2">#</th><th className="px-3 py-2">Customer</th><th className="px-3 py-2">Contact</th><th className="px-3 py-2">Plan</th><th className="px-3 py-2">From</th><th className="px-3 py-2">To</th><th className="px-3 py-2 text-center">Status</th><th className="px-3 py-2 text-right">Actions</th></tr>}>
            {rows.map((m) => (
              <tr key={m.id} className="text-slate-300">
                ...
                <td className="px-3 py-2 text-[12px] text-slate-400">
                  {m.customer_email ? <span className="block">{m.customer_email}</span> : null}
                  {m.customer_phone ? <span className="block">{m.customer_phone}</span> : null}
                  {!m.customer_email && !m.customer_phone ? <span className="text-slate-600">—</span> : null}
                </td>
            ...
            {rows.length === 0 && <EmptyRow colSpan={8} label="No memberships yet." />}
          </TableShell>
        </Card>
      )}
