...
function Inner({ wsId }: { wsId: string }) {
  const fetcher = useCallback(() => AccountingService.vendors.list(wsId), [wsId]);
  const { rows, loading, error, reload } = useList<VendorRow>(fetcher);
  ...
  const openEdit = (v: VendorRow) => {
    setEditing(v);
    setForm({ name: v.name, email: v.email || '', phone: v.phone || '', tax_id: v.tax_id || '', currency: v.currency || businessCurrency(), payment_terms: v.payment_terms || 'net_30' });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = editing
        ? await AccountingService.vendors.update(wsId, editing.id, form)
        : await AccountingService.vendors.create(wsId, form);
      if (!res.success) { setFormError(res.message || 'Could not save vendor.'); return; }
      setOpen(false); setForm(emptyForm); setEditing(null); reload();
    } catch (err) { setFormError(apiError(err, 'Could not save vendor.')); }
    finally { setSaving(false); }
  };

  return (
    <div className="space-y-5">
      <PageHeader title="Vendors" subtitle="Suppliers you receive bills from." action={<AddButton label="New vendor" onClick={openCreate} />} />
          ...
          <TableShell head={<tr><th className="px-3 py-2">Name</th><th className="px-3 py-2">Email</th><th className="px-3 py-2">Phone</th><th className="px-3 py-2">Terms</th><th className="px-3 py-2 text-center">Status</th><th className="px-3 py-2 text-right">Actions</th></tr>}>
            {rows.map((v) => (
              <tr key={v.id} className="text-slate-300">
                ...
                <td className="px-3 py-2 uppercase">{v.payment_terms}</td>
                <td className="px-3 py-2 text-center"><Pill>{v.is_active ? 'active' : 'inactive'}</Pill></td>
                <td className="px-3 py-2 text-right"><button onClick={() => openEdit(v)} className="text-xs font-medium text-cyan-300 hover:text-cyan-200">Edit</button></td>
              </tr>
            ))}
            {rows.length === 0 && <EmptyRow colSpan={6} label="No vendors yet." />}
          </TableShell>
        </Card>
      )}
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Edit vendor' : 'New vendor'}>
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <PrimaryButton type="submit" disabled={saving}>{saving ? 'Saving…' : editing ? 'Save changes' : 'Create vendor'}</PrimaryButton>
          </div>
        </form>
      </Modal>
    </div>
