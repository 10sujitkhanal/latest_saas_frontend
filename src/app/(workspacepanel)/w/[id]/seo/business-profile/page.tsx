  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold text-slate-900"><MapPin className="h-5 w-5 text-violet-500" /> Business Profile</h1>
          <p className="text-sm text-slate-500">Your name, address, phone, categories &amp; hours — what powers Google, Maps &amp; local search.</p>
        </div>
        <button onClick={save} disabled={saving} className="inline-flex items-center gap-1.5 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white hover:bg-violet-500 disabled:opacity-50">
          <Save className="h-4 w-4" /> {saving ? 'Saving…' : 'Save'}
        </button>
      </div>

      {p.is_connected ? (
        <div className="flex items-center gap-2 rounded-lg bg-emerald-50 px-4 py-2.5 text-sm font-medium text-emerald-700"><CheckCircle2 className="h-4 w-4" /> Connected to Google Business Profile — synced automatically.</div>
      ) : (
        <div className="rounded-lg bg-amber-50 px-4 py-2.5 text-sm text-amber-700">Manual mode — enter your details now. When Google Business Profile is connected, these sync automatically.</div>
      )}

      {toast && <div className={`rounded-lg px-4 py-2.5 text-sm font-medium ${toast.ok ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{toast.msg}</div>}
      ...
      <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
        <p className="font-semibold text-slate-900">Address (NAP)</p>
      ...
      <section className="space-y-2 rounded-xl border border-slate-200 bg-white p-4">
        <p className="flex items-center gap-1.5 font-semibold text-slate-900"><Clock className="h-4 w-4 text-violet-500" /> Opening hours</p>
        {p.hours.map((h, i) => (
          <div key={i} className="flex items-center gap-3 text-sm">
            <span className="w-10 font-medium text-slate-600">{DAYS[h.day] ?? DAYS[i]}</span>
            <label className="flex items-center gap-1.5 text-xs text-slate-500"><input type="checkbox" className="h-4 w-4 accent-violet-500" checked={!h.closed} onChange={(e) => setHour(i, { closed: !e.target.checked })} /> Open</label>
            {!h.closed && (
              <>
                <input type="time" className="rounded-lg border border-slate-200 px-2 py-1 text-sm" value={h.open} onChange={(e) => setHour(i, { open: e.target.value })} />
                <span className="text-slate-400">–</span>
                <input type="time" className="rounded-lg border border-slate-200 px-2 py-1 text-sm" value={h.close} onChange={(e) => setHour(i, { close: e.target.value })} />
              </>
            )}
