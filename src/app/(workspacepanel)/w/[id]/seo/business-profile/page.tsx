...
function Inner({ wsId }: { wsId: string }) {
  ...
  const input = 'w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-teal-400';
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold text-slate-900"><MapPin className="h-5 w-5 text-teal-500" /> Business Profile</h1>
      ...
      <section className="space-y-2 rounded-xl border border-slate-200 bg-white p-4">
        <p className="flex items-center gap-1.5 font-semibold text-slate-900"><Clock className="h-4 w-4 text-teal-500" /> Opening hours</p>
        {p.hours.map((h, i) => (
