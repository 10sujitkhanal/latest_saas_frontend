...
const STATUS_STYLE: Record<string, string> = {
  connected: 'bg-emerald-50 text-emerald-700', pending: 'bg-amber-50 text-amber-700',
  error: 'bg-red-50 text-red-700', disconnected: 'bg-slate-100 text-slate-500',
};
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="font-semibold text-slate-900">{PROVIDER_LABEL.google_business}</p>
      ...
      {reviews.length === 0 ? (
        <p className="rounded-xl border border-dashed border-slate-200 px-4 py-10 text-center text-sm text-slate-400"><MessageSquare className="mx-auto mb-2 h-6 w-6 text-slate-300" />No reviews yet. Add one manually, or connect Google to sync them.</p>
      ) : (
              ...
              <div key={rv.id} className="rounded-xl border border-slate-200 bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2"><span className="font-semibold text-slate-900">{rv.author_name || 'Anonymous'}</span><Stars n={rv.rating} /><span className="text-[11px] uppercase text-slate-400">{rv.platform}</span></div>
                ...
                {rv.reply_text && !p && (
                  <div className="mt-3 rounded-lg bg-slate-50 p-3">
                    ...
                    <p className="text-sm text-slate-700">{rv.reply_text}</p>
                  </div>
                )}
