...
export default function StorefrontPage({ params }: { params: Promise<{ schema: string }> }) {
  ...
  if (state === 'loading') {
    return <div className="min-h-screen grid place-items-center bg-white text-slate-400 text-sm">Loading store…</div>;
  }
  if (state === 'notfound' || !data) {
    return (
      <div className="min-h-screen grid place-items-center bg-white text-center px-6">
        <div>
          <div className="text-2xl font-bold text-slate-900">Store not available</div>
          <p className="mt-2 text-sm text-slate-500">This business hasn’t opened its storefront yet.</p>
        </div>
      </div>
    );
  }
